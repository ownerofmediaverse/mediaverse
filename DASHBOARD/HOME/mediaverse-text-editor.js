...
.composer-text-area.mv-te-move-mode .mv-te-drag-overlay{
  display:block;
}

.composer-text-area.has-bg.mv-te-move-mode .composer-input{
  position:absolute;
...
function wirePanel(){
 ...
 panel.querySelector('#mvTeMove').addEventListener('click', () => {
    ...
    if(style.moveMode) style.custom = true;   // ⭐ এটাই আসল fix
    syncUI(); applyToComposer();
...
if(!dragging) return;
style.custom = true;                 // ⭐ এইটা যোগ করবেন
